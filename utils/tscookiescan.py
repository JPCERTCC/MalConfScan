# Detecting TSCookie for Volatility
#
# LICENSE
# Please refer to the LICENSE.txt in the https://github.com/JPCERTCC/MalConfScan/
#
# How to use
# scan all processes:
# $ python3 vol.py -f images.mem -p [plugin_directory] tscookiescan
# specifiy scaning process with PID:
# $ python3 vol.py -f images.mem -p [plugin_directory] tscookiescan --pid [PID]

from . import basescan

import logging
import re
import pefile
from collections import OrderedDict
from struct import unpack_from, unpack
try:
    """import crypto libs here"""
    has_crypto = True
except ImportError:
    has_crypto = False

# logger for volatility
vollog = logging.getLogger(__name__)


MZ_HEADER = b"\x4D\x5A\x90\x00"


class tscookieConfig(basescan.baseConfig):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self._yara_sig = {
            'namespace1': 'rule TSCookie { \
                    strings: \
                       $v1 = "Mozilla/4.0 (compatible; MSIE 8.0; Win32)" wide\
                       $mz = { 4D 5A 90 00 } \
                       $b1 = { 68 D4 08 00 00 } \
                    condition: all of them}',
            'namespace2': 'rule TSC_Loader { \
                    strings: \
                       $v1 = "Mozilla/4.0 (compatible; MSIE 8.0; Win32)" wide\
                       $mz = { 4D 5A 90 00 } \
                       $b1 = { 68 78 0B 00 00 } \
                    condition: all of them}'
        }
        self._config_sig = [re.compile(b"\xC3\x90\x68\x00(...)\xE8(....)\x59\x6A\x01\x58\xC3", re.DOTALL),
                            re.compile(b"\x6A\x04\x68(....)\x8D(.....)\x56\x50\xE8", re.DOTALL),
                            re.compile(b"\x00\x00\x68(....)\xE8(....)\x59\x59\x6A\x01", re.DOTALL),
                            re.compile(b"\x68(....)\xE8(....)\x59\x6A\x01\x58\xC3", re.DOTALL),
                            re.compile(b"\x68(....)\xE8(....)\x59", re.DOTALL)]  # signature for searching configuration data
        self.CONNECT_MODE = {0: 'TCP', 1: 'HTTP with Credentials', 2: 'HTTP with Credentials', 3: 'HTTP with Credentials',
                             5: 'HTTP', 6: 'HTTPS', 7: 'HTTPS', 8: 'HTTPS'}
        self.PROXY_MODE = {0: 'Detect proxy settings', 1: 'Use config'}
        self.INJECTION_MODE = {0: 'Create process', 1: 'Injection running process'}
        self.PROCESS_NAME = {0: 'svchost.exe', 1: 'iexplorer.exe', 2: 'explorer.exe', 3: 'Default browser', 4: 'Setting process'}

    def rc4(self, data, key) -> bytes:
        x = 0
        box = bytearray(range(256))
        for i in range(256):
            x = (x + box[i] + key[i % len(key)]) % 256
            box[i], box[x] = box[x], box[i]
        x = 0
        y = 0
        buf = bytearray(data)
        for i in range(len(buf)):
            x = (x + 1) % 256
            y = (y + box[x]) % 256
            box[x], box[y] = box[y], box[x]
            buf[i] ^= box[(box[x] + box[y]) % 256]
        return bytes(buf)

    def parse_config(self, config):
        p_data = OrderedDict()
        for i in range(4):
            if config[0x10 + 0x100 * i] != "0x00":
                p_data["Server " + str(i)] = self.remove_unascii(unpack_from("<240s", config, 0x10 + 0x100 * i)[0])
                p_data["Server " + str(i) + " (port 1)"] = unpack_from("<H", config, 0x4 + 0x100 * i)[0]
                p_data["Server " + str(i) + " (port 2)"] = unpack_from("<H", config, 0x8 + 0x100 * i)[0]
        if config[0x400] != "\x00":
            p_data["Proxy server"] = self.remove_unascii(unpack_from("<128s", config, 0x400)[0])
            p_data["Proxy port"] = unpack_from("<H", config, 0x480)[0]
        p_data["ID"] = self.remove_unascii(unpack_from("<256s", config, 0x500)[0])
        p_data["KEY"] = "0x{0:X}".format(unpack_from(">I", config, 0x604)[0])
        if len(config) > 0x89C:
            p_data["Sleep time"] = unpack_from("<H", config, 0x89C)[0]

        return p_data

    def parse_loader_config(self, config):
        p_data = OrderedDict()
        p_data["Server name"] = unpack_from("<1024s", config, 0)[0]
        p_data["KEY"] = "0x{0:X}".format(unpack_from(">I", config, 0x400)[0])
        p_data["Sleep count"] = unpack_from("<H", config, 0x404)[0]
        p_data["Mutex"] = unpack_from("<32s", config, 0x40c)[0]
        mode = unpack_from("<H", config, 0x44c)[0]
        p_data["Connect mode"] = self.CONNECT_MODE[mode]
        p_data["Connect keep"] = unpack_from("<H", config, 0x454)[0]
        icmp = unpack_from("<H", config, 0x458)[0]
        if icmp == 100:
            p_data["ICMP mode"] = "Enable"
            p_data["ICMP bind IP"] = unpack_from("<330s", config, 0x4D4)[0]
        else:
            p_data["ICMP mode"] = "Disable"
        injection = unpack_from("<H", config, 0x624)[0]
        p_data["Injection mode"] = self.INJECTION_MODE[injection]
        p_data["Injection process name"] = self.PROCESS_NAME[unpack_from("<H", config, 0x628)[0]]
        p_data["Injection custom name"] = unpack_from("<256s", config, 0x62c)[0].replace("\0", "")
        if config[0x72c] != "\x00":
            p_data["Proxy server"] = unpack_from("<64s", config, 0x72c)[0]
            p_data["Proxy port"] = unpack_from("<H", config, 0x76c)[0]
            p_data["Proxy username"] = unpack_from("<64s", config, 0x770)[0]
            p_data["Proxy password"] = unpack_from("<64s", config, 0x790)[0]
        proxy = unpack_from("<H", config, 0x7b0)[0]
        p_data["Proxy mode"] = self.PROXY_MODE[proxy]
        p_data["AuthScheme"] = unpack_from("<H", config, 0x7b4)[0]

        return p_data

    def extract_config(self, data: bytes, malname: str, vad_base_addr: int) -> OrderedDict:
        """process dump data will be passed as data"""
        config_data = OrderedDict()

        dll_index = data.find(MZ_HEADER)
        if dll_index:
            dll_data = data[dll_index:]
            dll = pefile.PE(data=dll_data)

        if "TSCookie" in malname:
            d = 2
        else:
            d = 0

        for pattern in self._config_sig:
            mc = re.search(pattern, dll_data)
            if mc:
                try:
                    (config_rva, ) = unpack("=I", dll_data[mc.start() + d + 1: mc.start() + d + 5])
                    config_addr = dll.get_physical_by_rva(config_rva - dll.NT_HEADERS.OPTIONAL_HEADER.ImageBase)
                    break
                except BaseException:
                    vollog.info("[!] Faild to dump TSCookie configuration data.\n")

        config_size = 0
        enc = []
        while not (dll_data[config_addr + config_size] == 0x00 and dll_data[config_addr + config_size + 1] == 0x00 and dll_data[config_addr + config_size + 2] == 0x00):
            enc.append(dll_data[config_addr + config_size])
            config_size += 1

        if config_size == 0x8D4:
            rc4key_length = 4
        else:
            rc4key_length = 0x80

        enc_config = enc[rc4key_length:]
        rc4key = enc[:rc4key_length]
        config = self.rc4(enc_config, rc4key)
        try:
            if len(config) > 0:
                if "TSCookie" in malname:
                    config_data = self.parse_config(config)
                else:
                    config_data = self.parse_loader_config(config)
        except BaseException:
            vollog.info("[!] Faild to parse TSCookie configuration data.\n")

        return config_data
